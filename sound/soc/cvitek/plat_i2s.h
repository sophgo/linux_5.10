#ifndef __PLAT_I2S_H__
#define __PLAT_I2S_H__
...
#define MCLK_DIV_MASK          0x0000FFFF
#define BCLK_DIV_MASK          0xFFFF0000

#define CVI_I2S_PLAY	BIT(0)
#define CVI_I2S_RECORD	BIT(1)
#define CVI_I2S_SLAVE	BIT(2)
#define CVI_I2S_MASTER	BIT(3)
...
struct i2s_data_fmt_config {
	int ch_num;
	u32 data_size;
	u32 sample_rate;
};

struct i2s_dma_data_1 {
	void *data;
	dma_addr_t addr;
	u32 max_burst;
...
union i2s_snd_dma_data {
	struct i2s_dma_data_1 pd;
	struct snd_dmaengine_dai_dma_data dt;
};

#ifdef CONFIG_PM_SLEEP
struct i2s_reg_context {
	u32 blk_setting;
	u32 frame_setting;
	u32 slot_setting1;
	u32 slot_setting2;
...
struct i2s_obj {
	...
	union i2s_snd_dma_data play_dma_data;
	union i2s_snd_dma_data capture_dma_data;
	struct i2s_data_fmt_config config;

	struct snd_pcm_substream __rcu *tx_substream;
	int real_status;
	bool mclk_out;
#ifdef CONFIG_PM_SLEEP
	struct i2s_reg_context *reg_ctx;
	struct dwi2s_reg_context *reg_dwctx;
#endif
};
