...
static struct platform_device dummy_codec_dev = {
	.name         = "dummy_codec",
	.id       = -1,
	.dev = {
...
{
	platform_device_register(&dummy_codec_dev);
	platform_driver_register(&dummy_codec_driver);
	return 0;
}

static void dummy_codec_exit(void)
{
	platform_device_unregister(&dummy_codec_dev);
