...
static int pdm_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	struct pdm_obj *pdm = snd_soc_dai_get_drvdata(dai);
...
static int pdm_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	struct pdm_obj *pdm = snd_soc_dai_get_drvdata(dai);
...
static void pdm_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	struct pdm_obj *pdm = snd_soc_dai_get_drvdata(dai);
...
static int pdm_trigger(struct snd_pcm_substream *substream, int cmd, struct snd_soc_dai *dai)
{
	struct pdm_obj *pdm = snd_soc_dai_get_drvdata(dai);
	int ret = 0;
...
static const struct snd_soc_component_driver pdm_component_driver = {
	.name = "pdm_component",
};
...
static int pdm_probe(struct platform_device *pdev)
{
	...
	return devm_snd_soc_register_component(&pdev->dev, &pdm_component_driver,
					       &pdm_dai_driver, 1);
}

#ifdef CONFIG_OF
static const struct of_device_id pdm_of_match[] = {
	{ .compatible = "cvitek,cvitek_pdm", },
	{},
};
...
static struct platform_driver pdm_platform_driver = {
	.probe		= pdm_probe,
	.driver		= {
		.name	= "pdm_drvier",
...
module_platform_driver(pdm_platform_driver);

MODULE_DESCRIPTION("ASoC codec pdm driver");
MODULE_AUTHOR("Xiaodong <xiaodong.fan@sophgo.com>");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform: pdm_driver");
