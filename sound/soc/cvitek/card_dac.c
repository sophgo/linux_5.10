...
static int dac_card_hw_params(struct snd_pcm_substream *substream,
			      struct snd_pcm_hw_params *params)
{
	return 0;
}

static int dac_card_init(struct snd_soc_pcm_runtime *rtd)
{
	return 0;
}

static const struct snd_soc_ops dac_card_ops = {
	.hw_params = dac_card_hw_params,
};

static struct snd_soc_dai_link_component dac_card_cpus[] = {
	{
		.name = "4130000.i2s",
...
static struct snd_soc_dai_link_component dac_card_codecs[] = {
	{
		.name = "300a000.dac",
		.dai_name = "300a000.dac",
	},
};

static struct snd_soc_dai_link_component dac_card_platform[] = {
	{
		.name = "4130000.i2s",
		.dai_name = "4130000.i2s",
	},
...
static struct snd_soc_dai_link dac_card_dai_link[] = {
	{
		.name = "dai_codec_dac0",
		.stream_name = "dai_playback0",
		.cpus = &dac_card_cpus[0],
		.num_cpus = 1,
		.codecs = &dac_card_codecs[0],
		.num_codecs = 1,
		.platforms = &dac_card_platform[0],
		.num_platforms = 1,
		.ops = &dac_card_ops,
		.init = dac_card_init,
...
static struct snd_soc_card dac_card = {
	.owner = THIS_MODULE,
	.dai_link = dac_card_dai_link,
	.num_links = ARRAY_SIZE(dac_card_dai_link),
};

static const struct of_device_id card_dac_match_ids[] = {
	{
		.compatible = "cvitek,cvitek-card-dac",
		//.data = (void *) &cv182xa_adc_dai,
	},
	{ }
};
MODULE_DEVICE_TABLE(of, card_dac_match_ids);

static int dac_card_probe(struct platform_device *pdev)
{
	...
	if (np) {
		...
		for_each_child_of_node(np, dai_node) {
			...
			of_property_read_string(dai_node, "cpu_dai_name", &dac_card_cpus[idx].name);
			of_property_read_string(dai_node, "codec_dai_name", &dac_card_codecs[idx].name);
			of_property_read_string(dai_node, "platform_name", &dac_card_platform[idx].name);
			of_property_read_string(dai_node, "cpu_dai_name", &dac_card_cpus[idx].dai_name);
			of_property_read_string(dai_node, "codec_dai_name", &dac_card_codecs[idx].dai_name);
			of_property_read_string(dai_node, "platform_name", &dac_card_platform[idx].dai_name);
...
#ifdef CONFIG_PM
int dac_card_suspend(struct device *dev)
{
	return 0;
}

int dac_card_resume(struct device *dev)
{
	return 0;
}

int dac_card_poweroff(struct device *dev)
{
	return 0;
}

#else
#define dac_card_suspend	NULL
#define dac_card_resume		NULL
#define dac_card_poweroff	NULL
#endif

const struct dev_pm_ops dac_card_pm_ops = {
	.suspend = dac_card_suspend,
	.resume = dac_card_resume,
	.freeze = dac_card_suspend,
	.thaw = dac_card_resume,
	.poweroff = dac_card_poweroff,
	.restore = dac_card_resume,
};

static struct platform_driver dac_card_driver = {
	.driver = {
		.name = "dac_card_driver",
		.pm = &dac_card_pm_ops,
		.of_match_table = card_dac_match_ids,
	},
	.probe = dac_card_probe,
};

module_platform_driver(dac_card_driver);

MODULE_DESCRIPTION("ASoC DAC card driver");
MODULE_AUTHOR("Xiaodong <xiaodong.fan@sophgo.com>");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform: dac_card");
