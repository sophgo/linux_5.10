...
//        t.max = a->max;
	...
	if(refine) {
		...
		if(ret < 0) {
			pr_err("ret=%d rule_var:%d=%d, min:%d max:%d openmin:%d openmax:%d t_max %d t_min %d,a_empty:%d,t_empty:%d \n",
					ret,rule->deps[0],rule->var,a->min,a->max,a->openmin,a->openmax, t.max, t.min, a->empty,t.empty);
		}
		return ret;
	}
