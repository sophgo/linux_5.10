...
/**
 * cv184x_cooling_get_max_state - callback function to get the max cooling state.
 ...
 */
static int cv184x_cooling_get_max_state(struct thermal_cooling_device *cdev,
					unsigned long *state)
{
...
/**
 * cv184x_cooling_get_cur_state - function to get the current cooling state.
 * @cdev: thermal cooling device pointer.
 ...
 * Return: 0 (success)
 */
static int cv184x_cooling_get_cur_state(struct thermal_cooling_device *cdev,
					unsigned long *state)
{
	struct cv184x_cooling_device *cvcdev = cdev->devdata;
...
/**
 * cv184x_cooling_set_cur_state - function to set the current cooling state.
 * @cdev: thermal cooling device pointer.
 * @state: set this variable to the current cooling state.
 *
 * Callback for the thermal cooling device to change the cv184x cooling
 * current cooling state.
 *
 * Return: 0 on success, an error code otherwise.
 */
static int cv184x_cooling_set_cur_state(struct thermal_cooling_device *cdev,
					unsigned long state)
{
	struct cv184x_cooling_device *cvcdev = cdev->devdata;
	...
	dev_dbg(&cdev->device, "clk_tpu=%ld Hz\n", clk_get_rate(cvcdev->clk_tpu));
...
static struct thermal_cooling_device_ops const cv184x_cooling_ops = {
	.get_max_state = cv184x_cooling_get_max_state,
	.get_cur_state = cv184x_cooling_get_cur_state,
	.set_cur_state = cv184x_cooling_set_cur_state,
};

static struct cv184x_cooling_device *
cv184x_cooling_device_register(struct device *dev)
{
	struct thermal_cooling_device *cdev;
	struct cv184x_cooling_device *cvcdev = NULL;
	...
	cvcdev->clk_tpu = devm_clk_get(dev, "clk_tpu");
	if (IS_ERR(cvcdev->clk_tpu)) {
		dev_err(dev, "failed to get clk_tpu\n");
		return ERR_PTR(-EINVAL);
	}
...
static void cv184x_cooling_device_unregister(struct cv184x_cooling_device
						 *cvcdev)
{
	thermal_cooling_device_unregister(cvcdev->cdev);
}

static int cv184x_cooling_probe(struct platform_device *pdev)
{
	struct cv184x_cooling_device *cvcdev;

	cvcdev = cv184x_cooling_device_register(&pdev->dev);
	if (IS_ERR(cvcdev)) {
...
static int cv184x_cooling_remove(struct platform_device *pdev)
{
	struct cv184x_cooling_device *cvcdev = platform_get_drvdata(pdev);

	if (!IS_ERR(cvcdev))
		cv184x_cooling_device_unregister(cvcdev);

	return 0;
}

static const struct of_device_id cv184x_cooling_match[] = {
	{.compatible = "cvitek,cv184x-cooling"},
	{},
};
MODULE_DEVICE_TABLE(of, cv184x_cooling_match);

static struct platform_driver cv184x_cooling_driver = {
	.driver = {
		.name = "cv184x-cooling",
		.of_match_table = of_match_ptr(cv184x_cooling_match),
	},
	.probe = cv184x_cooling_probe,
	.remove = cv184x_cooling_remove,
};

module_platform_driver(cv184x_cooling_driver);

MODULE_AUTHOR("fisher.cheng@cvitek.com");
MODULE_DESCRIPTION("cv184x cooling driver");
MODULE_LICENSE("GPL");
