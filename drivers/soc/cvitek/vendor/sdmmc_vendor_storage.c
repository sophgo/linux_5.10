...
{
	return cvi_emmc_transfer(buffer, addr, n_sec << 9, write);
}
...
static int emmc_vendor_storage_init(void)
{
	...
	for (i = 0; i < EMMC_VENDOR_PART_NUM; i++) {
		...
		p_buf = (u8 *)g_vendor;
		if (cvi_emmc_transfer(p_buf, vendor_part_start +
	...
	if (max_ver) {
		if (emmc_vendor_ops((u8 *)g_vendor, vendor_part_start +
...
static int emmc_vendor_write(u32 id, void *pbuf, u32 size)
{
	...
	for (i = 0; i < item_num; i++) {
		...
		if (item->id == id) {
			...
			emmc_vendor_ops((u8 *)g_vendor, vendor_part_start +
				 ...
				 unsigned long arg)
{
	...
	case END_WRITE_SECTOR_IO:
	{
		...
		if (page_buf[0] <= (EMMC_BOOT_PART_SIZE * 512)) {
			...
			if (page_buf[1] != cvi_crc_32(g_idb_buffer, page_buf[0])) {
				ret = -2;
				goto exit;
			}
...
static int vendor_init_thread(void *arg)
{
	...
	if (!vendor_part_start) {
		struct cmdline_subpart *subpart = cmdline_subparts_find("VENDOR");

		if (subpart)
			vendor_part_start = subpart->from;
	}
	...
	if (!ret) {
		...
		cvi_vendor_register(emmc_vendor_read, emmc_vendor_write);
	} else {
		kfree(g_vendor);
		g_vendor = NULL;
	}
