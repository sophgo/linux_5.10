...
#include "../../../mmc/host/cvitek/cvi_sdmmc_ops.h"
#include <linux/mtd/ubi.h>

#define UBI_IDB_PART_OFFSET		64
#define UBI_SYS_PART_OFFSET		8064
#define UBI_BOOT_PART_SIZE		1024
#define UBI_VENDOR_PART_START		0
#define UBI_VENDOR_PART_SIZE		128
#define UBI_VENDOR_PART_NUM		1
#define UBI_VENDOR_TAG			0x524B5644
#define VENDOR_BLOCK_SIZE 512
/* don't work, need to fix */
// #define VENDOR_USE_IDB

struct cvi_emmc_transfer {
	u32 tag;
	u16 id;
	u16 len;
	u8 data[1024];
...
struct vendor_info {
	...
	u8	data[UBI_VENDOR_PART_SIZE * 512 - 1024 - 8];
	u32	hash;
	u32	version2;
};

#ifdef VENDOR_USE_IDB
...
static int vendor_ubi_num;
static DEFINE_MUTEX(vendor_ops_mutex);

const char *vendor_volname = "vendor_vol0";
static int ubi_vendor_op_read(u8 *buffer, u32 addr, u32 n_sec)
{
	...
	err = ubi_get_device_info(vendor_ubi_num, &di);
	if (err) {
		pr_info("ubi_get_device_info %d failed\n", vendor_ubi_num);
		return err;
	}

	desc = ubi_open_volume(vendor_ubi_num, 0, UBI_READONLY);
	if (IS_ERR(desc)) {
		pr_err("ubi_open_volume_nm %d failed\n", vendor_ubi_num);
		return PTR_ERR(desc);
	}

	ubi_get_volume_info(desc, &vi);
	if (strcmp(vi.name, vendor_volname) != 0) {
		pr_err("ubi_open_volume_nm %s failed\n", vendor_volname);
		return -1;
	}

	pr_info("read %s addr = %d, n_sec = %d\n", vi.name, addr, n_sec);

	err = ubi_read(desc, 0, buffer, addr * n_sec * VENDOR_BLOCK_SIZE, n_sec * VENDOR_BLOCK_SIZE);
	if (err)
		pr_err("ubi_leb_read failed with error %d\n", err);
...
static int ubi_vendor_op_write(u8 *buffer, u32 addr, u32 n_sec)
{
	struct ubi_volume_desc *desc = NULL;
	int err = -1;
	struct ubi_device_info di = {0};
	...
	err = ubi_get_device_info(vendor_ubi_num, &di);
	if (err) {
		pr_err("ubi_get_device_info %d failed\n", vendor_ubi_num);
		return err;
	}

	desc = ubi_open_volume(vendor_ubi_num, 0, UBI_READWRITE);
	if (IS_ERR(desc)) {
		pr_err("ubi_open_volume_nm %d failed\n", vendor_ubi_num);
		return PTR_ERR(desc);
	}
	...
	pr_info("change %s addr = %d, n_sec = %d\n", vi.name, addr, n_sec);

	err = ubi_leb_change(desc, 0, buffer, n_sec * VENDOR_BLOCK_SIZE);
	if (err)
		pr_err("ubi_leb_change failed with error %d\n", err);

	ubi_close_volume(desc);
	return err;
}

static int ubi_vendor_ops(u8 *buffer, u32 addr, u32 n_sec, int write)
{
	if (write)
		return ubi_vendor_op_write(buffer, addr, n_sec);
	else
		return ubi_vendor_op_read(buffer, addr, n_sec);
}

static int ubi_vendor_storage_init(void)
{
	...
	max_ver = 0;
	max_index = 0;
	for (i = 0; i < UBI_VENDOR_PART_NUM; i++) {
		p_buf = (u8 *)g_vendor;
		if (ubi_vendor_ops(p_buf, UBI_VENDOR_PART_START +
				 UBI_VENDOR_PART_SIZE * i, UBI_VENDOR_PART_SIZE, 0))
			goto error_exit;

		if (g_vendor->tag == UBI_VENDOR_TAG && g_vendor->version2 == g_vendor->version) {
	...
	if (max_ver) {
		if (ubi_vendor_ops((u8 *)g_vendor, UBI_VENDOR_PART_START +
				UBI_VENDOR_PART_SIZE * max_index,
				UBI_VENDOR_PART_SIZE, 0))
			goto error_exit;
	} else {
		memset((void *)g_vendor, 0, sizeof(*g_vendor));
		g_vendor->version = 1;
		g_vendor->tag = UBI_VENDOR_TAG;
...
static int ubi_vendor_read(u32 id, void *pbuf, u32 size)
{
	u32 i;
...
static int ubi_vendor_write(u32 id, void *pbuf, u32 size)
{
	...
	for (i = 0; i < item_num; i++) {
		...
		if (item->id == id) {
			...
			g_vendor->version2 = g_vendor->version;
			g_vendor->next_index++;
			if (g_vendor->next_index >= UBI_VENDOR_PART_NUM)
				g_vendor->next_index = 0;
			ubi_vendor_ops((u8 *)g_vendor, UBI_VENDOR_PART_START +
					UBI_VENDOR_PART_SIZE * next_index,
					UBI_VENDOR_PART_SIZE, 1);
			return 0;
		}
	}
...
static int id_blk_read_data(u32 index, u32 n_sec, u8 *buf)
{
	if (index + n_sec >= 1024 * 5)
		return 0;
	index = index + UBI_IDB_PART_OFFSET;

	return cvi_emmc_transfer(buf, index, n_sec << 9, 0);
}
...
static int id_blk_write_data(u32 index, u32 n_sec, u8 *buf)
{
	...
	return cvi_emmc_transfer(buf, index, n_sec << 9, 1);
}
...
static int emmc_write_idblock(u32 size, u8 *buf, u32 *id_blk_tbl)
{
	...
	u32 *p_check_buf = kmalloc(UBI_BOOT_PART_SIZE * 512, GFP_KERNEL);
	...
	for (i = 0; i < 5; i++) {
		...
		id_blk_write_data(UBI_BOOT_PART_SIZE * i, 1, (u8 *)p_check_buf);
		id_blk_write_data(UBI_BOOT_PART_SIZE * i + 1, totle_sec - 1, buf + 512);
		id_blk_write_data(UBI_BOOT_PART_SIZE * i, 1, buf);
		id_blk_read_data(UBI_BOOT_PART_SIZE * i, totle_sec, (u8 *)p_check_buf);
...
static u32 cvi_crc_32(unsigned char *buf, u32 len)
{
	u32 i;
	u32 crc = 0;
...
static long vendor_storage_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret = -1;
	int size;
	struct cvi_emmc_transfer *v_req;
	u32 *page_buf;
	...
	v_req = (struct cvi_emmc_transfer *)page_buf;
	...
	case VENDOR_READ_IO:
	{
		...
		if (v_req->tag == VENDOR_REQ_TAG) {
			size = ubi_vendor_read(v_req->id, v_req->data, v_req->len);
			if (size != -1) {
				v_req->len = size;
				ret = 0;
	...
	case VENDOR_WRITE_IO:
	{
		...
		if (v_req->tag == VENDOR_REQ_TAG && (v_req->len < 4096 - 8)) {
			...
			ret = ubi_vendor_write(v_req->id, v_req->data, v_req->len);
		}
	} break;
	...
	case WRITE_SECTOR_IO:
	{
		...
		if (!g_idb_buffer) {
			g_idb_buffer = kmalloc(4096 + UBI_BOOT_PART_SIZE * 512, GFP_KERNEL);
			if (!g_idb_buffer) {
				ret = -EFAULT;
				goto exit;
			}
	...
	case END_WRITE_SECTOR_IO:
	{
		...
		if (page_buf[0] <= (UBI_BOOT_PART_SIZE * 512)) {
			if (!g_idb_buffer) {
				ret = -EFAULT;
				goto exit;
			}
			if (page_buf[1] !=
				cvi_crc_32(g_idb_buffer, page_buf[0])) {
				ret = -2;
				goto exit;
			}
...
static int vendor_init_thread(void *arg)
{
	...
	pr_info("ubi_vendor_storage_init\n");
	do {
		ret = ubi_vendor_storage_init();
	...
	if (!ret) {
		...
		cvi_vendor_register(ubi_vendor_read, ubi_vendor_write);
	} else {
		kfree(g_vendor);
		g_vendor = NULL;
	}
...
static int __init vendor_storage_init(void)
{
	...
	vendor_ubi_num = ubi_get_device_by_part("VENDOR");
	if (vendor_ubi_num < 0) {
		pr_err("VENDOR partition is no attach to ubi device. Please run ubiattach\n");
		ret = vendor_ubi_num;
		goto err;
	}

	if (!find_vol_by_name(vendor_ubi_num, vendor_volname)) {
		pr_err("VENDOR vol %s is no exist. Please run ubimkvol\n", vendor_volname);
		ret = -ENODEV;
		goto err;
	}

	kthread_run(vendor_init_thread, (void *)NULL, "%s", __func__);
...
device_initcall(vendor_storage_init);
