...
typedef struct legacy_timer_emu {
	struct timer_list t;
	void (*function)(unsigned long emu);
	unsigned long data;
} _timer;

struct class *mon_class;
static dev_t mon_cdev_id;
struct proc_dir_entry *cvitek_dir;
struct proc_dir_entry *mon_proc_dir;
struct proc_dir_entry *bw_profiling;
struct proc_dir_entry *profiling_window_ms;

static u32 mon_window_ms = 20;
static u32 enable_mon_bw_profiling;
...
static int cvi_mon_probe(struct platform_device *pdev)
{
	...
	//create mon proc
	cvitek_dir = proc_mkdir("cvitek", NULL);
	if (!cvitek_dir)
		dev_err(&pdev->dev, "Error creating cvitek_dir proc folder entry\n");

	mon_proc_dir = proc_mkdir("mon", cvitek_dir);
	if (!mon_proc_dir)
		dev_err(&pdev->dev, "Error creating mon_proc_dir proc folder entry\n");

	bw_profiling = proc_create_data("bw_profiling", 0644, mon_proc_dir, &mon_bw_proc_ops, ndev);
	if (!bw_profiling)
		dev_err(&pdev->dev, "mon bw_profiling proc creation failed\n");

	profiling_window_ms = proc_create_data("profiling_window_ms", 0644, mon_proc_dir, &mon_window_proc_ops, ndev);
	if (!profiling_window_ms)
		dev_err(&pdev->dev, "mon profiling_window_ms proc creation failed\n");
