...
struct AXIMON_INFO_PORT {
	...
	uint64_t latency_write_avg_max;
	uint64_t latency_write_avg_min;
	uint64_t latency_read_avg;
	uint64_t latency_read_avg_max;
	uint64_t latency_read_avg_min;
...
static void axi_mon_count_port_info(uint32_t duration, uint32_t byte_cnt, struct AXIMON_INFO_PORT *axi_info)
{
	...
	pr_debug("duration=%d, byte_cnt=%d, count=%lld\n", duration, byte_cnt, axi_info->count);
	if (axi_info->count != 0) {
		if (bw) {
			if (!axi_info->bw_min)
				axi_info->bw_min = bw;
...
#if 0
	pr_err("%s bw=%d, bw_min=%lld, bw_max=%lld, bw_avg_sum=%lld\n",
...
void axi_mon_dump_single(struct AXIMON_INFO_PORT *port_info)
{
	...
	pr_err("\n");
	pr_err("%-8s bw_avg=%5dMB/s, bw_min=%5lldMB/s, bw_max=%5lldMB/s\n",
							port_info->port_name, (uint32_t)dividend,
							port_info->bw_min, port_info->bw_max);

	if (strcmp(port_info->port_name, "total") != 0) {
		pr_err("avg_write_latency=%lldns, avg_write_latency_min=%lldns, avg_write_latency_max=%lldns\n",
			divedend_w_lat, port_info->latency_write_avg_min, port_info->latency_write_avg_max);
		pr_err("avg_read_latency=%lldns, avg_read_latency_min=%lldns, avg_read_latency_max=%lldns\n",
	...
	if ((strcmp(port_info->port_name, "vip_rt") == 0) | (strcmp(port_info->port_name, "vc") == 0)) {
		pr_err("Write_Lat_histogram 0~10: %5lld,%5lld,%5lld,%5lld,%5lld,%5lld,%5lld,%5lld,%5lld,%5lld,%5lld.\n",
		...
		pr_err("Read_Lat_histogram 0~10: %5lld,%5lld,%5lld,%5lld,%5lld,%5lld,%5lld,%5lld,%5lld,%5lld,%5lld.\n",
...
void axi_mon_dump(void)
{
	...
	pr_err("%s profiling window time_avg=%3dms\n",
		dram_info.type, (uint32_t)aximon_info.total.time_avg / 1000);
...
#define CLK_DIVIDEND_MAGIC (0x1F2AB93105)
