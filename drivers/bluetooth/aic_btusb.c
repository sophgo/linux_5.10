...
int send_hci_cmd(firmware_info *fw_info)
{
	...
	if (g_chipid == PRODUCT_ID_AIC8801 ||
		...
		ret_val = usb_bulk_msg(fw_info->udev, fw_info->pipe_out,
				       fw_info->send_pkt, fw_info->pkt_len,
				       &len, 3000);
		if (ret_val || (len != fw_info->pkt_len)) {
			AICBT_INFO("Error in send hci cmd = %d,"
				   "len = %d, size = %d",
				   ret_val, len, fw_info->pkt_len);
		}
	} else if (g_chipid == PRODUCT_ID_AIC8800DC) {
		while ((ret_val < 0) && (i++ < 3)) {
			ret_val = usb_control_msg(fw_info->udev,
						  fw_info->pipe_out, 0,
						  USB_TYPE_CLASS, 0, 0,
...
int set_bt_onoff(firmware_info *fw_info, uint8_t onoff)
{
	...
	if (ret_val < 0) {
		AICBT_ERR("%s: Failed to send bt %s cmd, errno %d", __func__,
			  onoff != 0 ? "on" : "off", ret_val);
		return ret_val;
	}
...
int get_patch_addr_from_patch_table(firmware_info *fw_info, char *filename,
...
int patch_table_download(firmware_info *fw_info, char *filename)
{
	...
	while (p - rawdata < size) {
		printk("size = %d  p - rawdata = 0x%0lx \r\n", size,
		       p - rawdata);
		new = (struct aicbt_patch_table *)kmalloc(
			sizeof(struct aicbt_patch_table), GFP_KERNEL);
		...
		if ((cur->type) >= 1000) { //Temp Workaround
			...
			cur->data = (uint32_t *)kmalloc(
				sizeof(uint8_t) * cur->len * 8, GFP_KERNEL);
...
#endif
		if (sub_chip_id == 0) {
			ret_val = download_data(fw_info, FW_RAM_PATCH_BASE_ADDR,
						FW_PATCH_BASE_NAME);
			if (ret_val) {
			...
			ret_val = patch_table_download(fw_info,
						       FW_PATCH_TABLE_NAME);
			if (ret_val) {
			...
			uint32_t fw_ram_patch_base_addr =
				FW_RAM_PATCH_BASE_ADDR;

			ret_val = get_patch_addr_from_patch_table(
				fw_info, FW_PATCH_TABLE_NAME_U02,
				&fw_ram_patch_base_addr);
			if (ret_val) {
			...
			ret_val = get_patch_addr_from_patch_table(
				fw_info, FW_PATCH_TABLE_NAME_U02H,
				&fw_ram_patch_base_addr);
			if (ret_val) {
				printk("aic get patch addr fail %d\n", ret_val);
				goto free;
