...
irqreturn_t dw_spi_dma_transfer_handler(struct dw_spi *dws)
{
...
int dw_spi_dma_wait(struct dw_spi *dws, unsigned int len, u32 speed)
{
	...
	if (ms == 0) {
		pr_err("DMA transaction timed out,speed:%d\n", speed);
		return -ETIMEDOUT;
	}
...
int dw_spi_dma_config_tx(struct dw_spi *dws)
{
...
int dw_spi_dma_submit_tx(struct dw_spi *dws, struct scatterlist *sgl,
			 unsigned int nents)
{
...
int dw_spi_dma_config_rx(struct dw_spi *dws)
{
