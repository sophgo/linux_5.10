...
int maxio_self_check(struct phy_device *phydev,int checknum)
{
	...
	for(i = 0;i < checknum; i++)
	{
		...
		if(0 == ret)
		{
			dev_dbg(&phydev->mdio.dev, "MAE0621A READY\n");
			break;
		}
		else
		{
...
{
	dev_dbg(&phydev->mdio.dev, "maxio driver remove\n");
	if(phydev->priv!=NULL){
		kfree(phydev->priv);
	}
	phydev->priv=NULL;
...
static int maxio_mae0621a_config_init(struct phy_device *phydev)
{
	...
	dev_dbg(&phydev->mdio.dev, "MAXIO_PHY_VER: %s \n",MAXIO_PHY_VER);
...
static int maxio_mae0621a_probe(struct phy_device *phydev)
{
	...
	dev_dbg(&phydev->mdio.dev, "maxio_mae0621a_probe clkmode(oscillator) PHY_ID: 0x%x\n", phydev->phy_id);
	PHY_WRITE(phydev, MAXIO_PAGE_SELECT, 0x0);
	mdelay(100);
	return 0;
}
...
static int maxio_mae0621aq3ci_probe(struct phy_device *phydev)
{
	...
	dev_dbg(&phydev->mdio.dev, "maxio_mae0621aQ3C probe PHY_ID: 0x%x\n", phydev->phy_id);

#if IS_ENABLED(CONFIG_ARCH_CV186X)
	u32 sram_oem;
	void __iomem *oem_addess;

	oem_addess = ioremap(0x5207F80, 0x4);
	if (!oem_addess) {
