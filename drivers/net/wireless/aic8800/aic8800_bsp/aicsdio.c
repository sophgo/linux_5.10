...
#define CONFIG_PLATFORM_CVITEK 1
...
static int gpio_wifi_powerkey = -1;
#endif
...
#ifdef CONFIG_PLATFORM_CVITEK
	// pull up  power-key
	gpio_wifi_powerkey = cvi_get_wifi_pwr_on_gpio();
	ret = gpio_request(gpio_wifi_powerkey,"wifi_chip_powerkey");
	if (ret == 0) {
		gpio_direction_output(gpio_wifi_powerkey, 0);
		msleep(50);

		gpio_direction_output(gpio_wifi_powerkey, 1);
		msleep(10);

		printk("***reset aic8800dl powerkey ***  %d  ***\n", ret);
		gpio_free(gpio_wifi_powerkey);
	}

	// rescan mmc card
	if (cvi_sdio_rescan() != 0)
		printk("%s %d rescan err!\n",__func__, __LINE__);
	else
		printk("%s %d rescan ok \n", __func__, __LINE__);
#endif
