...
static int emmc_set_areatype(enum emmc_area_type areatype)
{
	int err;
	u8 part_config;
 ...
 */
static void emmc_prepare_mrq(struct mmc_request *mrq, struct scatterlist *sg,
		unsigned int sg_len, unsigned int dev_addr, unsigned int blocks, unsigned int blksz, int write)
{
...
static int emmc_busy(struct mmc_command *cmd)
{
 ...
 */
static int emmc_wait_busy(void)
{
	int ret, busy;
...
int cvi_emmc_transfer(u8 *buffer, unsigned int addr, unsigned int datasz, int write)
{
	...
	emmc_prepare_mrq(&mrq, &sg, 1, addr, datasz / BLKSZ, BLKSZ, write);
	...
	if (areatype != MMC_DATA_AREA_MAIN) {
		ret = emmc_set_areatype(MMC_DATA_AREA_MAIN);
		if (ret) {
			pr_err("emmc_set_areatype error!.\n");
			goto exit;
		}
	}
	...
	if (areatype != MMC_DATA_AREA_MAIN) {
		ret = emmc_set_areatype(areatype);
		if (ret)
			pr_err("emmc_set_areatype error!.\n");
	}
...
EXPORT_SYMBOL(cvi_emmc_transfer);
