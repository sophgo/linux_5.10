...
static struct cv184x_pll_hw_clock cv184x_pll_clks[] = {
	...
	CLK_G2_PLL(CV184X_CLK_CAM0PLL, "clk_cam0pll", cv184x_frac_pll_parent, REG_CAM0PLL_CSR,
		REG_CAM0PLL_SSC_SYN_CTRL, 0),
...
static struct cv184x_hw_clock cv184x_clks[] = {
	...
	CV184X_CLK(CV184X_CLK_RV1, "clk_rv1",
		...
		CLK_IGNORE_UNUSED),
	CV184X_CLK(CV184X_CLK_BUS, "clk_bus",		//for ap_fab
		((const char *[]) {"osc", "clk_mpll"}),
		REG_CLK_EN_0, 7,
	...
	CV184X_CLK(CV184X_CLK_GIC, "clk_gic",
		...
		0, -1,
		CLK_IS_CRITICAL),
	CV184X_CLK(CV184X_CLK_DBG, "clk_dbg",		//for common reset sync & debug_i2c
		((const char *[]) {"osc"}),
		REG_CLK_EN_0, 9,
		0, -1, 0, 0,							//25MHz
		0, -1, 0, 0,
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_SC, "clk_sc",			//for system counter
		((const char *[]) {"osc"}),
		REG_CLK_EN_0, 10,
		0, -1, 0, 0,							//25MHz
		0, -1, 0, 0,
	...
	CV184X_CLK(CV184X_CLK_USB20_CORECLKIN, "clk_usb20_coreclkin",
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_SD0, "clk_sd0",				//for sd0 card_clk
		((const char *[]) {"osc", "clk_cam1pll"}),
		REG_CLK_EN_1, 3,
	...
	CV184X_CLK(CV184X_CLK_100K_SD0, "clk_100k_sd0",		//for sd0 clk100K
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_SD1, "clk_sd1",				//for sd1 card_clk
		((const char *[]) {"osc", "clk_cam1pll"}),
		REG_CLK_EN_1, 5,
	...
	CV184X_CLK(CV184X_CLK_100K_SD1, "clk_100k_sd1",		//for sd1 clk100K
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_EMMC_CARD, "clk_emmc_card",	//for emmc card_clk
		((const char *[]) {"osc", "clk_cam1pll"}),
		REG_CLK_EN_1, 7,
		...
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_EMMC_100K, "clk_emmc_100K",	//for clk100k
		((const char *[]) {"clk_1M"}),
		REG_CLK_EN_1, 8,
		0, -1, 0, 0,
		0, -1, 0, 0,
	...
	CV184X_CLK(CV184X_CLK_PWM, "clk_pwm",
		...
		CLK_IS_CRITICAL),
	CV184X_CLK(CV184X_CLK_XTAL_MISC, "clk_xtal_misc",		// for 0-7 timer and other 100KHz clk
		((const char *[]) {"osc", "clk_fpll"}),
		REG_CLK_EN_1, 31,
	...
	CV184X_CLK(CV184X_CLK_APB_AUDSRC, "clk_apb_audsrc",		//for i2s_subsystem's pclk_audsrc
		...
		0, -1,
		CLK_IS_CRITICAL),
	// CV184X_CLK(CV184X_CLK_AHB_ROM, "clk_ahb_rom",		// for rom
	// 	((const char *[]) {"clk_hsperi"}),
	// 	REG_CLK_EN_3, 5,
	// 	0, -1, 0, 0,
	// 	0, -1, 0, 0,
	...
	// 	0, -1,
	// 	CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_AXI4_EMMC, "clk_axi4_emmc",		//for emmc core_clk
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_3, 6,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_AXI4_SD0, "clk_axi4_sd0",			//for sd0 core_clk
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_3, 7,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_AXI4_SD1, "clk_axi4_sd1",		//for sd1 core_clk
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_3, 8,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_SPI_NAND_GATE, "clk_spi_nand_gate",		//for spi_nand_top's clk_apb
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_3, 9,
		0, -1, 0, 0,
		0, -1, 0, 0,
	...
	// 	0, -1,
	// 	CLK_IS_CRITICAL),
	CV184X_CLK(CV184X_CLK_AHB_SF, "clk_ahb_sf",		//for apb_spic
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_3, 12,
		0, -1, 0, 0,
		0, -1, 0, 0,
	...
	CV184X_CLK(CV184X_CLK_APB_SPI3, "clk_apb_spi3",
		...
		0, -1,
		CVI_CLK_FLAG_FOR_OS),
	// CV184X_CLK(CV184X_CLK_UART0_GATE, "clk_uart0_gate",
	// 	((const char *[]) {"clk_hsperi"}),
	// 	REG_CLK_EN_3, 24,
	// 	0, -1, 0, 0,
	// 	0, -1, 0, 0,
	...
	// 	0, -1,
	// 	CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_APB_UART0, "clk_apb_uart0",
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_3, 25,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	// CV184X_CLK(CV184X_CLK_UART1_GATE, "clk_uart1_gate",
	// 	((const char *[]) {"clk_hsperi"}),
	// 	REG_CLK_EN_3, 26,
	// 	0, -1, 0, 0,
	// 	0, -1, 0, 0,
	...
	// 	0, -1,
	// 	CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_APB_UART1, "clk_apb_uart1",
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_3, 27,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	// CV184X_CLK(CV184X_CLK_UART2_GATE, "clk_uart2_gate",
	// 	((const char *[]) {"clk_hsperi"}),
	// 	REG_CLK_EN_3, 28,
	// 	0, -1, 0, 0,
	// 	0, -1, 0, 0,
	...
	// 	0, -1,
	// 	CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_APB_UART2, "clk_apb_uart2",
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_3, 29,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	// CV184X_CLK(CV184X_CLK_UART3_GATE, "clk_uart3_gate",
	// 	((const char *[]) {"clk_hsperi"}),
	// 	REG_CLK_EN_3, 30,
	// 	0, -1, 0, 0,
	// 	0, -1, 0, 0,
	...
	// 	0, -1,
	// 	CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_APB_UART3, "clk_apb_uart3",
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_3, 31,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	// CV184X_CLK(CV184X_CLK_UART4_GATE, "clk_uart4_gate",
	// 	((const char *[]) {"clk_hsperi"}),
	// 	REG_CLK_EN_4, 0,
	// 	0, -1, 0, 0,
	// 	0, -1, 0, 0,
	...
	// 	0, -1,
	// 	CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_APB_UART4, "clk_apb_uart4",
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_4, 1,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	CV184X_CLK(CV184X_CLK_APB_I2S0, "clk_apb_i2s0",
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_4, 2,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	CV184X_CLK(CV184X_CLK_APB_I2S1, "clk_apb_i2s1",
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_4, 3,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	CV184X_CLK(CV184X_CLK_APB_I2S2, "clk_apb_i2s2",
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_4, 4,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	CV184X_CLK(CV184X_CLK_APB_I2S3, "clk_apb_i2s3",
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_4, 5,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	CV184X_CLK(CV184X_CLK_AXI4_USB, "clk_axi4_usb",
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_4, 6,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	CV184X_CLK(CV184X_CLK_APB_USB, "clk_apb_usb",
		((const char *[]) {"clk_hsperi"}),
		REG_CLK_EN_4, 7,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	CV184X_CLK(CV184X_CLK_APB_I2C, "clk_apb_i2c",
		((const char *[]) {"clk_i2c"}),
		REG_CLK_EN_4, 8,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	CV184X_CLK(CV184X_CLK_APB_I2C0, "clk_apb_i2c0",
		((const char *[]) {"clk_apb_i2c"}),
		REG_CLK_EN_4, 9,
		0, -1, 0, 0,
		0, -1, 0, 0,
		0, -1,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	CV184X_CLK(CV184X_CLK_APB_I2C1, "clk_apb_i2c1",
		((const char *[]) {"clk_apb_i2c"}),
		REG_CLK_EN_4, 10,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	CV184X_CLK(CV184X_CLK_APB_I2C2, "clk_apb_i2c2",
		((const char *[]) {"clk_apb_i2c"}),
		REG_CLK_EN_4, 11,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	CV184X_CLK(CV184X_CLK_APB_I2C3, "clk_apb_i2c3",
		((const char *[]) {"clk_apb_i2c"}),
		REG_CLK_EN_4, 12,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	CV184X_CLK(CV184X_CLK_APB_I2C4, "clk_apb_i2c4",
		((const char *[]) {"clk_apb_i2c"}),
		REG_CLK_EN_4, 13,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	//peri system clk gate
	CV184X_CLK(CV184X_CLK_WGN, "clk_wgn",
		((const char *[]) {"clk_wgn_xclk"}),
		REG_CLK_EN_4, 14,
		0, -1, 0, 0,
		0, -1, 0, 0,
		0, -1,
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_WGN0, "clk_wgn0",
		((const char *[]) {"clk_wgn"}),
		REG_CLK_EN_4, 15,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_WGN1, "clk_wgn1",
		((const char *[]) {"clk_wgn"}),
		REG_CLK_EN_4, 16,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_WGN2, "clk_wgn2",
		((const char *[]) {"clk_wgn"}),
		REG_CLK_EN_4, 17,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_KEYSCAN, "clk_keyscan",
		((const char *[]) {"clk_keyscan_xclk"}),
		REG_CLK_EN_4, 18,
		0, -1, 0, 0,
		0, -1, 0, 0,
		0, -1,
		...
		0, -1,
		CVI_CLK_FLAG_FOR_OS),
	CV184X_CLK(CV184X_CLK_APB_WDT, "clk_apb_wdt",
		((const char *[]) {"clk_fab_100M"}),
		REG_CLK_EN_4, 19,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_IGNORE_UNUSED),
	CV184X_CLK(CV184X_CLK_TIMER0, "clk_timer0",
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_TIMER1, "clk_timer1",
		((const char *[]) {"clk_xtal_misc"}),
		REG_CLK_EN_4, 21,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_TIMER2, "clk_timer2",
		((const char *[]) {"clk_xtal_misc"}),
		REG_CLK_EN_4, 22,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_TIMER3, "clk_timer3",
		((const char *[]) {"clk_xtal_misc"}),
		REG_CLK_EN_4, 23,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_TIMER4, "clk_timer4",
		((const char *[]) {"clk_xtal_misc"}),
		REG_CLK_EN_4, 24,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_TIMER5, "clk_timer5",
		((const char *[]) {"clk_xtal_misc"}),
		REG_CLK_EN_4, 25,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_TIMER6, "clk_timer6",
		((const char *[]) {"clk_xtal_misc"}),
		REG_CLK_EN_4, 26,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_TIMER7, "clk_timer7",
		((const char *[]) {"clk_xtal_misc"}),
		REG_CLK_EN_4, 27,
		0, -1, 0, 0,
		0, -1, 0, 0,
	...
	CV184X_CLK(CV184X_CLK_SARADC, "clk_saradc",
		...
		0, -1,
		CVI_CLK_FLAG_FOR_OS),
	CV184X_CLK(CV184X_CLK_PM, "clk_pm",
		((const char *[]) {"osc"}),
		REG_CLK_EN_4, 30,
		0, -1, 0, 0,
		0, -1, 0, 0,
		...
		0, -1,
		CLK_SET_RATE_GATE),
	CV184X_CLK(CV184X_CLK_APB_GPIO, "clk_apb_gpio",
		((const char *[]) {"clk_fab_100M"}),
		REG_CLK_EN_4, 31,
		0, -1, 0, 0,
		0, -1, 0, 0,
	...
	CV184X_CLK(CV184X_CLK_OENC, "clk_oenc",
		((const char *[]) {"clk_raw_axi"}),
		REG_CLK_EN_5, 0,						//reg_clk_dummy0_en
		0, -1, 0, 0,
		0, -1, 0, 0,
		0, -1,
		0, -1,
