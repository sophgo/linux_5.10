...
static int fts_irq_registration(struct fts_ts_data *ts_data)
{
    ...
    ts_data->irq = tpd_gpio_as_int(ts_data->pdata->irq_gpio);
...
static int fts_power_source_init(struct fts_ts_data *ts_data)
{
    ...
    FTS_FUNC_ENTER();
    ts_data->vdd = regulator_get(gp_tpd->tpd_dev, "vtouch");
    if (IS_ERR_OR_NULL(ts_data->vdd)) {
...
static void fts_platform_data_init(struct fts_ts_data *ts_data)
{
    ...
    if (!gpio_is_valid(pdata->irq_gpio)) {
        FTS_ERROR("Parse IRQ GPIO from dt failed %d", pdata->irq_gpio);
        pdata->irq_gpio = -1;
    } else {
        FTS_INFO("IRQ GPIO: %d", pdata->irq_gpio);
    }
    /* reset gpio num */
    pdata->reset_gpio = of_get_named_gpio(np, "touch,reset-gpio", 0);
    if (!gpio_is_valid(pdata->reset_gpio)) {
        FTS_ERROR("Parse RST GPIO from dt failed %d", pdata->reset_gpio);
        pdata->reset_gpio = -1;
    } else {
