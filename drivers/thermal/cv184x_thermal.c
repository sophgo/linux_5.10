...
struct cv184x_thermal_zone {
	unsigned int ch;
	void __iomem *base;
	struct cv184x_thermal *ct;
};

struct cv184x_thermal {
	struct device *dev;
	void __iomem *base;
	struct clk *clk_tempsen;
};

static void cv184x_thermal_init(struct cv184x_thermal *ct)
{
...
static void cv184x_thermal_uninit(struct cv184x_thermal *ct)
{
	void __iomem *base = ct->base;
	u32 regval;
...
static int cv184x_read_temp(void *data, int *temperature)
{
	struct cv184x_thermal_zone *ctz = data;
...
static const struct thermal_zone_of_device_ops cv184x_thermal_ops = {
	.get_temp = cv184x_read_temp,
};

static const struct of_device_id cv184x_thermal_of_match[] = {
	{
		.compatible = "cvitek,cv184x-thermal",
	},
	{},
};
MODULE_DEVICE_TABLE(of, cv184x_thermal_of_match);

static int cv184x_thermal_probe(struct platform_device *pdev)
{
	struct cv184x_thermal *ct;
	struct cv184x_thermal_zone *ctz;
	struct resource *res;
...
static int cv184x_thermal_remove(struct platform_device *pdev)
{
	struct cv184x_thermal *ct = platform_get_drvdata(pdev);

	cv184x_thermal_uninit(ct);
...
static int cv184x_thermal_suspend(struct device *dev)
{
	struct cv184x_thermal *ct = dev_get_drvdata(dev);
...
static int cv184x_thermal_resume(struct device *dev)
{
	struct cv184x_thermal *ct = dev_get_drvdata(dev);
...
static SIMPLE_DEV_PM_OPS(cv184x_thermal_pm_ops,
			 cv184x_thermal_suspend, cv184x_thermal_resume);

static struct platform_driver cv184x_thermal_driver = {
	.probe = cv184x_thermal_probe,
	.remove = cv184x_thermal_remove,
	.driver = {
		.name = "cv184x-thermal",
		.pm = &cv184x_thermal_pm_ops,
		.of_match_table = cv184x_thermal_of_match,
	},
};

module_platform_driver(cv184x_thermal_driver);
MODULE_DESCRIPTION("cv184x thermal driver");
